...
@WebServlet("/cumController1")
public class cumController1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		session.setAttribute("origcum", origin);
		session.setAttribute("destcum", destination);
		...
		try {
			...
			else
			{
				...
				session.setAttribute("distorg", y);
				session.setAttribute("timeorg", z);
				if(y>60)
				{
				...
				else
				{
					...
					response.sendRedirect("dropapi.jsp");
				}
			}
